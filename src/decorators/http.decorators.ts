...
import { ApiBasicAuth, ApiBearerAuth, ApiCreatedResponse,
    ApiOkResponse, ApiOperation, ApiResponse, ApiSecurity
} from "@nestjs/swagger";
...
export const ApiPublic = (options: IApiPublicOptions = {}): MethodDecorator=>{
    ...
    const errorResponses = (options.errorResponses || defaultErrorResponse)?.map(
